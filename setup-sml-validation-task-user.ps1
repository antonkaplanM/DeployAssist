# Setup Windows Task Scheduler for SML Validation Processing (Current User)
# This script creates a user-level scheduled task (no admin required)
...
Write-Host ""
Write-Host "SML Validation Processing - User Task Setup" -ForegroundColor Cyan
...
# Register the task for current user (no admin required)
Write-Host "Registering scheduled task for current user..." -ForegroundColor Cyan
try {
...
Write-Host "  User:      Current user (no admin needed)" -ForegroundColor White
...
Write-Host "The task will run every 10 minutes automatically." -ForegroundColor Green
Write-Host ""
